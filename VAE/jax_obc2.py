...
if __name__ == '__main__':
    ...
    fname_dcd = 'trajectory_com_aligned.dcd'
    ...
    #grad_gbsa_obc2_fn = jax.value_and_grad (ener_gbsa_obc2_fn)
    l_pdb = False
    if l_pdb:
        u = mda.Universe (fname_pdb)
        x_Ai = jnp.array (u.atoms.positions)*jnp.float32(0.1) 
        
        born_radii = get_born_radii (x_Ai)
        
        print (born_radii.shape)
        print (born_radii[:5])
        
        en_obc2 = ener_gbsa_obc2_fn (x_Ai, born_radii)
    else:
        ...
        x_com = jnp.array (x_com)*jnp.float32(0.1)

        born_radii = jax.vmap (get_born_radii) (x_com)
        en_obc2 = jax.vmap(ener_gbsa_obc2_fn) (x_com, born_radii)
        #en_obc2, grad = jax.vmap (grad_gbsa_obc2_fn) (x_com, born_radii)
    

    print ('en_obc2', en_obc2/4.184)
