"""
Train a Perceiver VAE from a JSON configuration file.

Usage:
    python scripts/train_perceiver.py <json_file>

The Perceiver VAE uses cross-attention from a small set of latent query
vectors to the full atom token sequence, scaling as O(n_atoms) rather than
O(n_atoms^2).  Uses the standard Experiment trainer.

JSON config extras (on top of standard keys):
    embed_dim        : int   – embedding width (default min(256, input_size))
...
from deepmms.models import PerceiverVAE
from deepmms.training.loss import atom_rmsd
from deepmms.utils import printf


def main(json_fn):
    """Entry point: parse args and run the PerceiverVAE training pipeline."""
    start = datetime.now()
    exp = Experiment(json_fn, from_json_params=False, model_cls=PerceiverVAE)
    exp.MAIN_train(n_epochs=1000, verbose=100)
    train_time = datetime.now() - start

    printf(
        f"Time to train {exp.n_latents} latent PerceiverVAE for {exp.epoch} epochs was"
    ...
    title = f"{exp.n_latents} Latents - PerceiverVAE RMSD Loss"
    plt.title(title)
    ...
    title = "PerceiverVAE Reconstruction RMSD"
    plt.title(title)
    plt.xlabel("RMSD (Angstrom)")
    plt.savefig(os.path.join(figure_dir, title + ".png"), dpi=900, bbox_inches="tight")

    printf(f"Summary: PerceiverVAE latents={exp.n_latents},"
           f" final_test_rmsd={rmsd_test[-1]*10:.3f} A")


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python scripts/train_perceiver.py <json_file>")
        sys.exit(1)
    main(sys.argv[1])
