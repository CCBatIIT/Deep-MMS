...
#dcd_fns = ['/media/volume/Josephs-Volume/Wasim/compresstraj/examples/protein/hewl.xtc']
...
#top_fns = ['/media/volume/Josephs-Volume/Wasim/compresstraj/examples/protein/hewl.pdb']
top_fns = ['Simulation/1crn_H.prmtop']

if os.path.basename(os.getcwd()) == 'Deep-MMS':
    dcd_fns = [os.path.join(os.getcwd(), fn) if fn.startswith('Simulation') else fn for fn in dcd_fns ]
    top_fns = [os.path.join(os.getcwd(), fn) if fn.startswith('Simulation') else fn for fn in top_fns]
