...
def get_ener_harm_fun(bonds, bond_types, rb0, kb0):
    '''
...
def get_ener_angle_fun(angles, angle_types, r_theta0, k_theta0):
    '''
...
def get_ener_torsion_fun(torsions, torsion_types, torsion_values):
    '''
...
def get_ener_bonded_fun(prm_raw_data):
    '''
    ...
    ener_harm_fun = get_ener_harm_fun(bonds, bond_types, r_bond0, k_bond0)

    angles, angle_types, r_theta0, k_theta0 = get_angles_info(prm_raw_data)
    ener_angle_fun = get_ener_angle_fun(angles, angle_types, r_theta0, k_theta0)

    dihedrals, dihedral_types, dihedral_values = get_dihedrals_info(prm_raw_data)
    ener_torsion_fun = get_ener_torsion_fun(dihedrals, dihedral_types, dihedral_values)

    def compute_fn(R):
        return ener_harm_fun(R) + ener_angle_fun(R) + ener_torsion_fun(R)
...
def get_ener_nonbonded14_fun(atom_types, nonbonds, sigma, epsilon, chgs):
    """
...
def get_ener_nonbonded_pair_fun(atom_types, nonbonds, sigma, epsilon, chgs):
...
def get_torsion_value_fun(torsions, torsion_types, torsion_values):
    '''
    torsions: jnp.array ((n_torsion, 4), dtype=int) 
    torsion_types: jnp.array ( (n_torsion), dtype=int )
    n_theta0 : jnp.array ( (n_torsion_type), dtype=int )
...
def get_amber_functions(fname_prmtop):
    #Load the prmtop
    ...
    #Develop energy functions
    ener_bonded_fun = jax.jit(get_ener_bonded_fun(prm_raw_data))
    ener_nbond_fun = jax.jit(get_ener_nonbonded_pair_fun(atom_types, nonbond_pairs, sigma, epsilon, chgs))
    ener_nbond14_fun = jax.jit(get_ener_nonbonded14_fun(atom_types, nbonds14, sigma, epsilon, chgs))

    #Stuff for torsions
    dihedrals, dihedral_types, dihedral_values = get_dihedrals_info(prm_raw_data)
    torsion_fun = jax.jit(get_torsion_value_fun(dihedrals, dihedral_types, dihedral_values))
    
    #Define the gas energy function
    @jax.vmap
    def gas_energy_function(R):
        ...
        #print(R.shape)
        R = R.reshape(-1,3)
        en_bonded = ener_bonded_fun(R) # R is frame
        en_lj, en_chg = ener_nbond_fun(R)
        en_lj14, en_chg14 = ener_nbond14_fun(R)
    ...
    def torsional_values(R):
        ...
        #print(R.shape)
        R = R.reshape(-1,3)
        return torsion_fun(R) # R is frame
    
    return gas_energy_function, torsional_values